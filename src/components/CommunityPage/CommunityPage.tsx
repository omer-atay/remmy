...
export function CommunityPage({ name }: { name: string }) {
  ...
  const [, setClickedCommunityNames] = useLocalStorage<string[]>('clickedCommunityNames', []);
  ...
  useEffect(() => {
    ...
    const name = community.community_view.community.local
  ...
  }, [community, setClickedCommunityNames]);
